...
function Register() { // כמו שנצטרך לעשות בעבודה אבל יש עוד אופציה בregister2. אביה השתמשה בזה שכאן לתרגיל לדוגמה לעבודה.
    ...
    const register =()=>{ // ככה חיברנו את השרת שמחובר לדאטה בייס
        // המתודה הזאת עושה בקשה עם הנתיב המתאים במוגדר במתודה מתאימה בקונטרולר בצד שרת, שמחברת את הלקוח לשרת, ובתוך המתודה ההיא מקשרים את מתודת השאילתא ממחלקת הדאטה בייס - ככה מקשרים את הדאטה בייס לשרת ואז ליוזר
        //http: - פרוטוקול
